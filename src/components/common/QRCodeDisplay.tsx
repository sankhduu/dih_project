...
import { QrCode, Download, Check, Copy } from 'lucide-react';
...
export function QRCodeDisplay({
  ...
  useEffect(() => {
    let active = true;
    if (value) {
      generateQRCodeDataUrl(value).then((src) => {
        if (active && src) {
          setQrSrc(src);
        }
      });
      ...
      {showDetails && (
        <div className="mt-3 w-full text-center">
          ...
          <div className="mt-2 flex items-center justify-center gap-2">
            <button
              onClick={handleCopy}
              className="px-2.5 py-1 text-xs font-medium text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-md transition-colors flex items-center gap-1 cursor-pointer"
